...
/*
    ...
    public void CheckStudent(Context context1) {
        PrimesysTrack.mDbHelper = DBHelper.getInstance(context1);
...
//		StudentId=Integer.parseInt(myAdapter.getItem(0).getId());
    ...
    {
        String url=Common.URL+"ParentAPI.asmx/GetPaymentTypeAPP";
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                System.out.println("PaymetReq-----------"+postParameter.toString());
    ...
    void parseData(String result)
    {
        try
        {
            ...
            for(int i=0;i<array.length();i++){
                ...
                type.setId(parseInt(jo.getString("Id")));
                type.setAmountOfPlan(parseInt(jo.getString("AmountOfPlan")));
    ...
    private void settypedata(ArrayList<PaymentTypeDTO> typelist) {
        ...
        {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                new UserInfoTask().executeOnExecutor(
                        AsyncTask.THREAD_POOL_EXECUTOR, (Void)null);
            } else {
    ...
    {
        String url=Common.URL+"ParentAPI.asmx/GetPaymentInfo";
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                postParameter.add(new BasicNameValuePair("ParentId",Common.userid));
                post.setEntity(new UrlEncodedFormEntity(postParameter));
                System.out.println("UserPaymetinfoReq-----------"+postParameter.toString());
                HttpResponse response = httpclient.execute(post);
                result = EntityUtils.toString(response.getEntity());
            } catch (Exception e) {
                result = e.getMessage();
    ...
    public void parseUserData(String result) {
        try
        {
            ...
            text_status.setText(jo.getString("ActiveStatus"));
