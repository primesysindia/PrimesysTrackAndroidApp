...
public class GeofencingUpdatedrawCircle extends AppCompatActivity implements  OnMapReadyCallback,SeekBar.OnSeekBarChangeListener {
...
*/
				...
				if(geoname.getText().length()!=0){
					if(st_in.isChecked()||st_out.isChecked()&& alert_app.isChecked()||alert_email.isChecked())
					{
						...
						geoObj.setDiscripation("");
							...
							if (KMeterselect>=0.5) {
							LoginActivity.mClient.sendMessage(make_fenceUpdate_JSON());
						}else
	...
	private void FindById() {
        ...
        context = GeofencingUpdatedrawCircle.this;
        myFragmentManager = getSupportFragmentManager();

		myMapFragment = (SupportMapFragment) myFragmentManager.findFragmentById(R.id.mapfragment);
     ...
     //   Common.ShowSweetAlert("Tap on map to select Geo-fencing location.", context);
	...
	public void onMapReady(GoogleMap googleMap) {
		...
		if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) ==
			...
			if(getIntent().getSerializableExtra("GeoObj")!=null)
			{
				geoObj=(GeofenceDTO) getIntent().getSerializableExtra("GeoObj");
				if (geoObj.getLang()!=null&&geoObj.getLat()!=null)
	...
	private void setgeolocation(GeofenceDTO geoObjexist)
	{
		...
		activemode.setChecked(Boolean.parseBoolean(geoObj.getEnable()));

		updateMarkerWithCircleradius((int) (Double.parseDouble(geoObj.getDistance()))/100);
	}
	...
	private void drawMarkerWithCircle(LatLng position){
	 ...
	 @Override  
		...
		private void updateMarkerWithCircleradius(int radius) {
			...
			Log.e("KMMMMMMMMMM------",KMeterselect+"");
	...
	protected String make_fenceNew_JSON() {
		...
		protected String make_fenceUpdate_JSON() {
	...
	public static void parseNewFenceJSON(String result) {
		try
		{
			...
			if (jo.getString("event").equals("geofence_added_successfully")) {
				Common.ShowSweetSucess(context, "Geo-fence added successfully.");
	...
	public static void parseUpdateFenceJSON(String result) {
		try
		{
			...
			if (jo.getString("event").equals("geofence_updated_successfully")) {
				...
				pDialog.setContentText("Geo-fence update successfully.");
